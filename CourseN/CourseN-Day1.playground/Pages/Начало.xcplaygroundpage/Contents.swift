...
let numbers_a = backpack1 + backpack2
...
func трехчлен(x: Int) -> Int {
    let plus5 = x + 5
    let sq = квадрат(plus5)
    return sq - 10 * x - 25
}
...
2.23547659666876587568758765876566.description
...
numbers.map(удвоить).map(утроить)[2] == 31 * 6
...
numbers.map(удвоить).reduce(0, combine: +)
